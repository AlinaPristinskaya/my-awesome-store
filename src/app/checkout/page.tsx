...
import { 
  ...
  CheckCircle2, CreditCard, Banknote, Landmark, FileText 
} from "lucide-react";
...
function CheckoutContent() {
  ...
  const handlePlaceOrder = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (result.success) {
      // --- ЛОГІКА ДЛЯ WAYFORPAY (БЕЗ ПОМИЛОК unknown) ---
      if (result.paymentData && paymentType === 'WAYFORPAY') {
        const form = document.createElement('form');
        form.method = 'POST';
        form.action = 'https://secure.wayforpay.com/pay';
        form.acceptCharset = 'utf-8';

        // Явно вказуємо Record<string, any>, щоб TypeScript не сварився
  ...
  if (isOrdered) return (
    <div className="min-h-screen flex flex-col items-center justify-center p-6 text-center bg-white text-black">
      ...
      <p className="text-gray-500 mb-6 font-bold">Ваше замовлення успішно прийнято.</p>
      <Link href="/" className="bg-black text-white px-10 py-4 rounded-2xl font-black uppercase text-xs hover:bg-indigo-600 transition-all">На головну</Link>
    </div>
  );
            ...
            <form onSubmit={handlePlaceOrder} className="space-y-6">
              ...
              <div className="bg-white p-8 rounded-[2.5rem] border border-gray-100 shadow-sm">
                ...
                {deliveryService === 'NP' ? (
                  ...
                  <input name="address" placeholder="Адреса (Індекс, область, місто, вулиця)..." className="w-full p-4 bg-gray-50 rounded-2xl outline-none font-bold" />
                )}
              </div>
